    ...
    private void iniAchievements() {
        ...
        try {
            RoomDatabase.databaseExecutor.execute(() -> {
                db.achievementDao().insert(new Achievement(1, R.drawable.ic_verified_24px, "Tutorial done", "We can start now!", true));
                db.achievementDao().insert(new Achievement(2, R.drawable.ic_alarm_on_24px, "Ring! Ring!", "Reminds you of the important things!", false));
                db.achievementDao().insert(new Achievement(3, R.drawable.ic_star_24px, "Rookie", "Your first entry!", false));
                db.achievementDao().insert(new Achievement(4, R.drawable.ic_emoji_nature_24px, "Busy Bee", "Created three entries in a row.", false));
                db.achievementDao().insert(new Achievement(5, R.drawable.ic_timeline_24px, "Wow!", "Seven entries in a row", false));
                db.achievementDao().insert(new Achievement(6, R.drawable.ic_timeline_good_24px, "Workaholic!", "One month Pocket Diary used regularly!", false));
            });
        } catch (Exception e) {
